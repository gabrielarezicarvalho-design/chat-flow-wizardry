...
import { Loader2, Eye, EyeOff, Send, CheckCircle, Users, BarChart3, Bot, Target, TrendingUp, Zap, ArrowLeft, Shield } from 'lucide-react';
  ...
  >
    ...
    <div className="relative z-10">
      <div
        ...
        style={{
          background: `linear-gradient(135deg, ${pc}, ${lightenHex(pc, 30)})`,
          boxShadow: `0 4px 14px ${hexToRgba(pc, 0.3)}`,
        }}
      >
...
const LoginForm = ({ partner, showPassword, setShowPassword, pc, bgSoft }: {
  partner: PartnerConfig; showPassword: boolean; setShowPassword: (v: boolean) => void; pc: string; bgSoft: string;
}) => (
  <motion.div
  ...
  >
    ...
    <p className="text-center text-xs mt-6" style={{ color: hexToRgba('#ffffff', 0.25) }}>
      Preview apenas — para login real, acesse pelo domínio configurado.
    </p>
  </motion.div>
);
...
const WhiteLabelPreview = () => {
  ...
  // Generate a rich dark background derived from the primary color
  const bgBase = `hsl(${hsl.h}, ${Math.min(hsl.s, 25)}%, 8%)`;
  const bgCard = hexToRgba(pc, 0.04);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: `
            radial-gradient(ellipse 80% 60% at 20% 40%, ${hexToRgba(pc, 0.07)}, transparent 70%),
      ...
      >
        <div className="relative z-10 flex flex-col justify-center w-full p-12 xl:p-16 mt-12">
          ...
          <div className="flex items-center gap-5 mb-10">
            {partner.logo_url ? (
              ...
              <motion.div
                ...
                style={{
                  background: `linear-gradient(135deg, ${pc}, ${lightenHex(pc, 30)})`,
                  boxShadow: `0 8px 32px ${hexToRgba(pc, 0.3)}`,
                }}
                animate={{ y: [0, -5, 0] }}
                transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }}
              >
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mb-10">
            ...
            <div className="grid grid-cols-2 gap-3">
              {statsCards.map((stat, i) => (
                <StatsCard key={i} stat={stat} index={i} pc={pc} bgSoft={bgSoft} />
              ))}
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
            <h2 className="text-sm font-semibold uppercase tracking-wider mb-4" style={{ color: hexToRgba('#ffffff', 0.5) }}>
              Recursos Principais
            </h2>
            <div className="space-y-2.5">
              {features.map((feat, i) => (
                <FeatureRow key={i} feat={feat} index={i} pc={pc} bgSoft={bgSoft} />
              ))}
            </div>
          </motion.div>
        </div>
      ...
      <div className="w-full lg:w-2/5 flex items-center justify-center p-6 lg:p-10 relative mt-12">
        ...
        <div
          ...
          style={{
            background: `radial-gradient(ellipse 70% 50% at 50% 50%, ${hexToRgba(pc, 0.06)}, transparent 70%)`,
          }}
        />
        ...
        <div className="w-full max-w-sm relative z-10">
          ...
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.2 }} className="lg:hidden text-center mb-8">
            {partner.logo_url ? (
              ...
              <div
                className="w-20 h-20 rounded-2xl flex items-center justify-center text-3xl font-bold text-white mx-auto mb-4"
          ...
          <LoginForm partner={partner} showPassword={showPassword} setShowPassword={setShowPassword} pc={pc} bgSoft={bgSoft} />
        </div>
      </div>
    </div>
  );
